 ...
 * 2. Applies in-memory rate limiting per IP
...
// ====== Rate Limit Settings ======
const RATE_LIMIT_WINDOW = 60 * 1000; // Time window: 1 minute
const RATE_LIMIT_MAX = 300;          // Maximum requests per IP in the window
...
export async function middleware(req) {
  ...
  if (
    suspiciousPatterns.some(
      (regex) => regex.test(pathname) || regex.test(searchParams.toString())
    )
  ) {
...
/**
 * Apply middleware globally, excluding static assets and favicon.
 */
export const config = {
